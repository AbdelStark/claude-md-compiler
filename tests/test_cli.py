...
def test_cli_check_command_returns_json_violations(tmp_path):
    ...
    assert (
        payload["violations"][0]["explanation"]
...
def test_cli_check_command_accepts_absolute_paths(tmp_path):
    ...
    result = subprocess.run(
        [
            ...
            str(target),
            "--write",
            str((target / "src/main.py").resolve()),
            "--json",
        ],
        capture_output=True,
        text=True,
...
def test_cli_fix_command_returns_versioned_json_plan(tmp_path):
    ...
    assert payload["next_action"] == ("Read at least one required context path before keeping changes to src/main.py: docs/rfcs/**.")
