        ...
        normalized_remediations.append(
            {
                ...
                "source_block_id": _optional_string(remediation.get("source_block_id"), field=f"remediations[{index}].source_block_id"),
...
def _render_text(plan: dict[str, Any]) -> str:
    ...
    for index, remediation in enumerate(plan["remediations"], start=1):
        lines.append(f"{index}. [{remediation['priority']}] {remediation['rule_id']} ({remediation['kind']}): {remediation['message']}")
