...
def _require_string(value: Any, *, field: str, context: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise ValueError(f"{context} requires a string '{field}'")
    return value.strip()
...
def _parse_event(event: Any, *, index: int) -> ExecutionInputs:
    ...
    if not isinstance(kind, str) or not kind.strip():
        raise ValueError(f"events[{index}] must contain a string 'kind'")
    kind = kind.strip()
    if kind not in ALLOWED_EVENT_KINDS:
        allowed = ", ".join(sorted(ALLOWED_EVENT_KINDS))
        raise ValueError(f"events[{index}] kind {kind!r} is unsupported; expected one of: {allowed}")
...
def load_execution_inputs_file(path: Path | str) -> ExecutionInputs:
    ...
    try:
        ...
        raise ValueError(f"execution input payload file is not valid JSON: {exc}") from exc
...
def load_execution_inputs_text(text: str, *, source: str = "stdin") -> ExecutionInputs:
    ...
    try:
        ...
        raise ValueError(f"execution input payload from {source} is not valid JSON: {exc}") from exc
    return load_execution_inputs(payload)
