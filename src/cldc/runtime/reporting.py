...
def _normalize_violation(value: Any, *, index: int) -> dict[str, Any]:
    ...
    return {
        ...
        'recommended_action': _require_string(
            value.get('recommended_action'), field=f'violations[{index}].recommended_action'
        ),
        ...
        'source_block_id': _optional_string(
            value.get('source_block_id'), field=f'violations[{index}].source_block_id'
        ),
    }
...
def _render_text(report: dict[str, Any]) -> str:
    ...
    for index, violation in enumerate(report['violations'], start=1):
        lines.append(
            f"{index}. [{violation['mode']}] {violation['rule_id']} ({violation['kind']}): {violation['message']}"
        )
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    if git_metadata:
        if git_metadata.get('mode') == 'staged':
            ...
            git_summary = (
                f"{git_metadata.get('base')}...{git_metadata.get('head')} "
                f"({git_metadata.get('write_path_count', 0)} changed paths)"
            )
