...
| Symptom                                     | Cause                          | Fix                                    |
|---------------------------------------------|--------------------------------|----------------------------------------|
| ModuleNotFoundError: cldc                   | Not installed in editable mode | Run `pip install -e ".[dev]"`          |
| pytest: no tests collected                  | Wrong working directory        | Run from repo root                     |
| KeyError on rule field                      | Missing required field in YAML | Check rule has `id` and `kind` fields  |
| Duplicate rule ID error                     | Same ID in multiple sources    | Use unique IDs across all policy files |
| FileNotFoundError on compile                | Invalid repo path              | Pass directory containing CLAUDE.md    |

</known_issues>

<recovery_patterns>
  1. Read the full error traceback — cldc errors include source file and line info
  2. Check that test fixtures exist: tests/fixtures/repo_a/
  3. Re-install: pip install -e ".[dev]"
  4. Run pytest -v to identify which specific test fails
</recovery_patterns>
</troubleshooting>
...
Modular skills in .codex/skills/ (symlinked at .claude/skills/ and .agents/skills/):
  ...
  — rfc-implementation.md: How to implement features from RFC specs
  — testing.md: Testing patterns and conventions for cldc
  — debugging.md: Debugging and troubleshooting workflows
  — rule-engine.md: Adding new rule kinds and validation logic
</skills>

<memory>
<project_decisions>
  2026-04-04: Spec-first development — RFCs define contracts before implementation — Ensures deterministic, well-tested output
  2026-04-04: PyYAML as sole dependency — Minimize dependency surface for a security-adjacent tool — Considered ruamel.yaml (heavier), toml (less expressive)
  2026-04-04: Lockfile as JSON — Universal readability, easy diffing in PRs — Considered YAML (ambiguous), binary (opaque)
  2026-04-04: Pipeline architecture (ingest→parse→compile) — Clean separation enables independent testing and extension — Considered monolithic compiler (harder to test)
</project_decisions>

<lessons_learned>
  — Source ordering must be deterministic — sort all glob results to ensure reproducible lockfiles
  — Inline ```cldc``` blocks in CLAUDE.md are extracted as separate PolicySource entries with provenance
  — Rule IDs must be globally unique across all sources — parser enforces this
</lessons_learned>
</memory>
