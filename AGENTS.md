# Codex-md-compiler Agent Context

## Project Identity

`cldc` compiles repository policy from `AGENTS.md` and related YAML files into `.Codex/policy.lock.json`, then enforces that policy over explicit runtime evidence and git-derived diffs.
...
- Treat `.Codex/policy.lock.json`, policy report JSON, and fix-plan JSON as versioned contracts.
...
- bundled preset policy packs (`default`, `strict`, `docs-sync`) reachable via `extends:` in `.Codex-compiler.yaml` and inspectable with `cldc preset list`/`cldc preset show`
